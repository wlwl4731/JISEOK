...
def divide_left_right(lines):
	...
	for j in range(len(slopes)):
		...
		# OpenCV 좌표계에서는 아래방향으로 y가 증가하므로 기울기 부호가 반대다
...
def get_line_params(lines):
	...
	# 허프변환 함수의 결과로 여러개의 선분이 검출되기 때문에 기울기의 평균값을 이용해 차선 구한다.
	m = m_sum / size
	b = y_avg - m * x_avg
...
def get_line_pos(lines, left=False, right=False):
	...
	if m == 0 and b == 0:
		...
		y = Gap / 2 # ROI좌표에서 y값 중앙으로 고정 = 20
		pos = (y - b) / m # x좌표

		# ROI영역이 아닌 전체 영상에 차선 그려주기
		# ROI좌표계에서 얻어진 좌표를 전체 영상 좌표로 바꿔줘야된다.
		# (x1, 480)과 (x2, 320) 두 점을 잇는 파란색 선을 그린다. (process_image함수에서 그린다)
		b += Offset
		x1 = (Height - b) / float(m)
...
def process_image(frame):
	...
	# (x1, 480)과 (x2, 320) 두 점을 잇는 파란색 선을 그린다.
